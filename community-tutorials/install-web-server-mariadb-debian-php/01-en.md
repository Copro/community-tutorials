...
- Press <kbd>CTRL + W</kbd> and <kbd>ENTER</kbd>: `post_max_size = 8M`.
  - Replace this line with `post_max_size = 10G`. This defines the maximum file size of a POST Body when data is uploaded which might include multiple files. You can set this value lower if you don't plan to upload multiple files which exceed the 10 Gigabyte limit (for example, 100M for a maximum of 100 Megabyte for a multiple file upload. This value must be the same size or greater than the next one `upload_max_filesize`)<br>
- Press <kbd>CTRL + W</kbd> and <kbd>ENTER</kbd>: `upload_max_filesize = 2M`.
  - Replace this line with `upload_max_filesize = 10G`. This defines the maximum file size of a single files that can still be uploaded. You can set this value lower if you do not plan to upload single files bigger than 10 Gigabyte (for example, 100M for a maximum of 100 Megabyte for a single file). <br>
